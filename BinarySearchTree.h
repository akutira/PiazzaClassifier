...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    if (empty_impl(node)) {
      return nullptr;
        ...
        return node; // datum found
    } else if (less(query, node->datum)) {
        return find_impl(node->left, query, less); // Recurse left
    } else if (!less(query, node->datum)){
        return find_impl(node->right, query, less); // Recurse right
    } else {
      return nullptr;
    }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if (empty_impl(node)){
      return true;
    } else if (!empty_impl(node->left) && less(node->datum, (node->left)->datum)){
      return false;
    } else if (!empty_impl(node->right) && less((node->right)->datum, node->datum)){
      return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (empty_impl(node)){
      ...
      if (!empty_impl(left_result) && less(left_result->datum, node->datum)) {
        return left_result;
      } else {
        return node;
      }
