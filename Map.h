...
private:
  ...
  class PairComp {  //OH
    public:
      Key_compare comp;
      bool operator()(Pair_type one, Pair_type two) const{
        //Key_compare compare;
...
public:
  ...
  Iterator find(const Key_type& k) const{
    ...
    temp.first = k;     //if tree.find() != nullptr?? 
    temp.second = Value_type(); //does this return it's value or?
  ...
  Value_type& operator[](const Key_type& k){
    ...
    if(temp == tree.end()){ //change to nullptr?
      std:: pair<Key_type, Value_type> temp1;
      temp1.first = k;
      temp1.second = Value_type();
      temp = tree.insert(temp1);
      return temp->second;
    }
    else{
      return temp->second;
